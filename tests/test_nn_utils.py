...
def test_cross_entropy():
    ...
    numpy.testing.assert_allclose(
        run_cross_entropy(inputs.view(-1, inputs.size(-1)), targets.view(-1)).detach().cpu().numpy(),
    ...
    numpy.testing.assert_allclose(
        run_cross_entropy(large_inputs.view(-1, large_inputs.size(-1)), targets.view(-1)).detach().cpu().numpy(),
        large_expected_cross_entropy.detach().cpu().numpy(),
        atol=1e-4,
    )
